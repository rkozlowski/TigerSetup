...
use crate::Result;
use crate::manifest::{DependencyEntry, LoadedManifest};
...
/// The record a declaration yields before any catalog is consulted.
pub fn declared(entry: &DependencyEntry) -> Dependency {
...
pub fn resolve(loaded: &LoadedManifest, offline: bool) -> Result<ResolvedDependencies> {
    ...
    for entry in &loaded.manifest.dependencies {
        let mut dependency = declared(entry);
