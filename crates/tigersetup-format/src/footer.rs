//! The fixed 128-byte trailer that maps the installer file.
//!
...
//! 16  8   metadata offset
//! 24  8   metadata length
//! 32  8   payload offset
//! 40  8   payload length
//! 48  32  SHA-256 of the metadata block
//! 80  32  SHA-256 of the payload block
//! 112 4   reserved (zero)
//! 116 4   CRC-32 (IEEE) of bytes [0, 116)
//! 120 8   trailing magic "PTSREGIT"
//! ```
...
impl Footer {
    pub fn encode(&self) -> [u8; FOOTER_LEN] {
        ...
        out[16..24].copy_from_slice(&self.metadata_offset.to_le_bytes());
        out[24..32].copy_from_slice(&self.metadata_length.to_le_bytes());
        out[32..40].copy_from_slice(&self.payload_offset.to_le_bytes());
        out[40..48].copy_from_slice(&self.payload_length.to_le_bytes());
        out[48..80].copy_from_slice(&self.metadata_sha256);
        out[80..112].copy_from_slice(&self.payload_sha256);
        let crc = crc32fast::hash(&out[..CRC_OFFSET]);
        out[CRC_OFFSET..CRC_OFFSET + 4].copy_from_slice(&crc.to_le_bytes());
        out[120..128].copy_from_slice(&MAGIC_TAIL);
        out
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Footer, FormatError> {
        ...
        if bytes[0..8] != MAGIC_HEAD || bytes[120..128] != MAGIC_TAIL {
            return Err(FormatError::new(
                "footer_missing",
        ...
        let mut metadata_sha256 = [0u8; 32];
        metadata_sha256.copy_from_slice(&bytes[48..80]);
        let mut payload_sha256 = [0u8; 32];
        payload_sha256.copy_from_slice(&bytes[80..112]);
        Ok(Footer {
            format_major,
            format_minor,
            metadata_offset: read_u64(16),
            metadata_length: read_u64(24),
            payload_offset: read_u64(32),
            payload_length: read_u64(40),
            metadata_sha256,
            payload_sha256,
        })
    }

    /// Checks that the blocks the footer names lie inside the file, in order,
    /// and end where the footer begins.
    pub fn check_layout(&self, footer_offset: u64) -> Result<(), FormatError> {
        ...
        match (metadata_end, payload_end) {
            (Some(me), Some(pe))
                if self.metadata_offset > 0
                    && me == self.payload_offset
                    && pe == footer_offset
...
///
/// The rule is positional: `file_len - 128`, unless the executable carries an
...
/// tests use a stub engine) falls back to the end-of-file rule.
...
mod tests {
    ...
    fn footer_round_trips() {
        ...
        assert!(footer.check_layout(6200).is_ok());
        assert!(footer.check_layout(6201).is_err());
    }
    ...
    fn footer_is_at_end_of_file_without_signature() {
        assert_eq!(locate_footer(10_000, &fake_pe(0, 0)).unwrap(), 10_000 - 128);
        assert_eq!(locate_footer(10_000, b"not a pe").unwrap(), 10_000 - 128);
    }
