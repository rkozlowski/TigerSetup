...
`tigersetup-setup.exe`, `tigersetup-loader.exe` and the installed help. The
release workflow builds it once from the release commit on the release-quality
path, checks it against the engine and loader of that build, and publishes it
as a GitHub Release asset at a stable, version-specific public URL, with
its WinGet manifest set, generated by the same §8.2 workflow TigerSetup offers
every consumer, on those same bytes, and a record of every file's SHA-256 and
